...
module.exports = (req, res, next) => {
  ...
  if (!authorization) {
    return res.status(400).json({ status: 0, session: "expired", error: "You must be logged in" });
  }
  const token = authorization.replace("Bearer ", "");
  jwt.verify(token, JWT_KEYS, (err, payload) => {
    if (err) {
      return res.status(400).json({ status: 0, session: "expired", error: "Token expired, Please logged in" });
    }
    const { id } = payload;
